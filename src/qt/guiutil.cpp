...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("fazhanchain"))
        return false;
...
{
    // Convert fazhanchain:// to fazhanchain:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("fazhanchain://"))
    {
        uri.replace(0, 12, "fazhanchain:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "fazhanchain.lnk";
}
...
{
    return GetAutostartDir() / "fazhanchain.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("fazhanchain-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  fazhanchain-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("fazhanchain-Qt"));
